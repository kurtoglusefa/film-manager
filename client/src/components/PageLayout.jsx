...
function PublicToReviewLayout(props) {
  ...
  const deleteFilm = film =>
    API.deleteFilm(film).then(() => setDirty(true)).catch(handleErrors);

  const updateFilm = film =>
    API.updateFilm(film).then(() => setDirty(true)).catch(handleErrors);
  ...
  return (
    <>
      ...
      <FilmToReviewTable
        ...
        /* IMPORTANT: use the handler passed down from App.jsx */
