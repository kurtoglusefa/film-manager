...
// }
const getPrivateFilms = async (filmManager, pageNumber) => {
  // ignore filmManager/path paging for now, your backend already knows the URL
  let path = SERVER + '/api/films/me';
  if (pageNumber !== undefined) path += '?page=' + pageNumber; // optional

  return getJson(fetch(path, { credentials: 'include' })).then(json => {
    const items = json.items ?? [];
    const totalItems = json.total ?? items.length;

    // fake pagination info in sessionStorage, so the UI widgets still work
    ...
    return items.map(film =>
      new Film({
        ...
        owner: parseInt(film.owner),
        privateFilm: film.private,
...
// }
const getPublicFilms = async (filmManager, pageNumber) => {
  let path = SERVER + '/api/films/public';
  if (pageNumber !== undefined) path += '?page=' + pageNumber;
...
// }
const getPublicFilmsToReview = async (filmManager, pageNumber) => {
  let path = SERVER + '/api/films/reviews/me';
  if (pageNumber !== undefined) path += '?page=' + pageNumber;
  ...
  return getJson(fetch(path, { credentials: 'include' })).then(json => {
    ...
    return items.map(film => new Film(film));
  });
};
...
// }
const getFilmReviews = async (film, pageNumber) => {
  let path = SERVER + film.reviews;  // e.g. /api/films/public/3/reviews
  if (pageNumber !== undefined) path += '?page=' + pageNumber;
  ...
  return getJson(fetch(path, { credentials: 'include' })).then(json => {
    ...
    return items.map(r => new Review(r));
  }).catch(err => {
    sessionStorage.setItem('totalPages', 0);
    sessionStorage.setItem('currentPage', 0);
    sessionStorage.setItem('totalItems', 0);
...
async function selectFilm(film, user) {
  return getJson(
    fetch(`${SERVER}/api/users/${user.id}/selection`, {
      method: 'PUT',
